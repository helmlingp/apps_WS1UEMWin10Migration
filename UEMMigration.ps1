...
function InstallWS1App {
    ...
    #Copy bundle to the folder
    Copy-Item -Path "AirWatchLLC.VMwareWorkspaceONE" -Destination $WS1Apppath -Recurse

    #find bundle and install it
...
<#         Write-Host "Getting Device ID from WS1 UEM"
        ...
        # Remove Uninstall Commands for Enterprise Apps & Copy to _Backup key. Keeps the apps on the device.
        $WS1AppDeployPath = "HKLM:\SOFTWARE\AirWatchMDM\AppDeploymentAgent\AppManifests"
        $WS1Apps = Get-ChildItem -Path $WS1AppDeployPath
        foreach ($app in $ws1app) {
            Rename-ItemProperty -Path $WS1AppDeployPath -Name "DeploymentManifestXML" -NewName "DeploymentManifestXML_Backup"
            New-ItemProperty -Path $WS1AppDeployPath$app -Name "DeploymentManifestXML"
        }
        #Not needed if uninstalling the Agent as it does it for us
<#         # Call Enterprise Wipe
        ...
        $StatusMessageLabel.Text = "Removing iHub & WSONE Application"
        Start-Sleep -Seconds 1
